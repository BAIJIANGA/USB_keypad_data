...
/* Private macro -------------------------------------------------------------*/
#define LS_CS(x) x?HAL_GPIO_WritePin(GPIOD,GPIO_PIN_8,GPIO_PIN_SET):HAL_GPIO_WritePin(GPIOD,GPIO_PIN_8,GPIO_PIN_RESET)
/* Private variables ---------------------------------------------------------*/

/* Private functions ---------------------------------------------------------*/
void LSM6DS3_Init()
{
  uint8_t value = 0X80|0x04;
  //重置芯片
  LSM6DS3_ACC_GYRO_WriteReg(&hspi1, LSM6DS3_ACC_GYRO_CTRL3_C, &value, 1);
  HAL_Delay(100);
  value |= 0X01;
  //软件复位芯片 设置为高性能模式
  LSM6DS3_ACC_GYRO_WriteReg(&hspi1, LSM6DS3_ACC_GYRO_CTRL3_C, &value, 1);
  ...
  //使能加速度X Y Z 轴
  value = 0x38;
  LSM6DS3_ACC_GYRO_WriteReg(&hspi1, LSM6DS3_ACC_GYRO_CTRL9_XL, &value, 1);
  HAL_Delay(50);
  //使能陀螺仪X Y Z 轴
  value = 0x38;
  LSM6DS3_ACC_GYRO_WriteReg(&hspi1, LSM6DS3_ACC_GYRO_CTRL10_C, &value, 1);
  HAL_Delay(50);
  //设置加速度计输出速率为1.66k
  LSM6DS3_ACC_GYRO_W_ODR_XL(&hspi1,LSM6DS3_ACC_GYRO_ODR_XL_1660Hz);
  //设置加速度计量程为+-8g 16位数据
  LSM6DS3_ACC_GYRO_W_FS_XL(&hspi1,LSM6DS3_ACC_GYRO_FS_XL_2g);
  HAL_Delay(50);
  //设置陀螺仪输出速率为1.66k
  LSM6DS3_ACC_GYRO_W_ODR_G(&hspi1,LSM6DS3_ACC_GYRO_ODR_G_1660Hz);
  HAL_Delay(50);
  //设置陀螺仪量程为+-2000dps 16位数据
  LSM6DS3_ACC_GYRO_W_FS_G(&hspi1,LSM6DS3_ACC_GYRO_FS_G_500dps);
  HAL_Delay(50);
  //使能块更新
  LSM6DS3_ACC_GYRO_W_BDU(&hspi1,LSM6DS3_ACC_GYRO_BDU_BLOCK_UPDATE);
  HAL_Delay(50);
}

void LSM6DS3_Data_Conversion (LSM6DS3_DateTypeDef * LSM6DS3_Data)
{
    LSM6DS3_Data->ACC_x_y_z[0] = LSM6DS3_Data->ACC_Data[0]*ACC_SCALE_FACTOR;
    LSM6DS3_Data->ACC_x_y_z[1] = LSM6DS3_Data->ACC_Data[1]*ACC_SCALE_FACTOR;
    LSM6DS3_Data->ACC_x_y_z[2] = LSM6DS3_Data->ACC_Data[2]*ACC_SCALE_FACTOR;
    
    LSM6DS3_Data->GYRO_p_r_y[0] = LSM6DS3_Data->GYRO_Data[0]*GYRO_SCALE_FACTOR;
    LSM6DS3_Data->GYRO_p_r_y[1] = LSM6DS3_Data->GYRO_Data[1]*GYRO_SCALE_FACTOR;
    LSM6DS3_Data->GYRO_p_r_y[2] = LSM6DS3_Data->GYRO_Data[2]*GYRO_SCALE_FACTOR;

    LSM6DS3_Data->GYRO_p_r_y[0] -= LSM6DS3_Data->GYRO_Zero_Shift[0];
    LSM6DS3_Data->GYRO_p_r_y[1] -= LSM6DS3_Data->GYRO_Zero_Shift[1];
    LSM6DS3_Data->GYRO_p_r_y[2] -= LSM6DS3_Data->GYRO_Zero_Shift[2];
    
    ACC_LIMIT(LSM6DS3_Data->ACC_x_y_z[0]);
    ACC_LIMIT(LSM6DS3_Data->ACC_x_y_z[1]);
    ACC_LIMIT(LSM6DS3_Data->ACC_x_y_z[2]);
}

void LSM6DS3_Get_Gyro_Zero_Shift (LSM6DS3_DateTypeDef * LSM6DS3_Data,unsigned int len)
{
  float data[3];
    for(unsigned int a = 0;a < len;a++)
    {
      LSM6DS3_ACC_GYRO_GetRawAccData(data,(uint8_t*)LSM6DS3_Data->ACC_Data);
      LSM6DS3_ACC_GYRO_GetRawGyroData(data,(uint8_t*)LSM6DS3_Data->GYRO_Data);
      LSM6DS3_Data_Conversion(LSM6DS3_Data);
      data[0] += LSM6DS3_Data->GYRO_p_r_y[0];
      data[1] += LSM6DS3_Data->GYRO_p_r_y[1];
      data[2] += LSM6DS3_Data->GYRO_p_r_y[2];
      HAL_Delay(10);
    }
    LSM6DS3_Data->GYRO_Zero_Shift[0] = data[0]/len;
    LSM6DS3_Data->GYRO_Zero_Shift[1] = data[1]/len;
    LSM6DS3_Data->GYRO_Zero_Shift[2] = data[2]/len;
}
/************** Generic Function  *******************/
...
status_t LSM6DS3_ACC_GYRO_WriteReg(void *handle, u8_t Reg, u8_t *Bufp, u16_t len)
{
  ...
  LS_CS(0);
  uint8_t  newReg = Reg&0x7F;//|0x01 << 7;
  HAL_SPI_Transmit(&hspi1,&newReg,1,6000);
  HAL_SPI_Transmit(&hspi1,Bufp,len,6000);
  LS_CS(1);
  return MEMS_SUCCESS;
}
...
status_t LSM6DS3_ACC_GYRO_ReadReg(void *handle, u8_t Reg, u8_t *Bufp, u16_t len)
{
  ...
  LS_CS(0);
  uint8_t  newReg = Reg|0x80;//0x01 << 7;
  HAL_SPI_Transmit(&hspi1,&newReg,1,60000);
  HAL_SPI_Receive(&hspi1,Bufp,len,60000);
  LS_CS(1);
  return MEMS_SUCCESS; 
}
